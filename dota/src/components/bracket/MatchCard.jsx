...
export function describeBracketToken(value) {
  ...
  const blastElim = text.match(/^(PI|LC|BPO)(\d+)M(\d+)([WL])$/);
  if (blastElim) {
    const bracketLabel =
      blastElim[1] === "PI" ? "Play-In" : blastElim[1] === "LC" ? "Last Chance" : "main playoffs";
    const role = blastElim[4] === "W" ? "Winner" : "Loser";
    return `${role} of BLAST ${bracketLabel}, ${bracketTokenRoundLabel(blastElim[2])}, match ${blastElim[3]} — feeds the next slot in that bracket.`;
  }
  ...
  if (bpi)
    return `Play-In entrant #${bpi[1]}. After groups finish, slots map from the merged remainder pool (cross-group wins + Neustadtl): best Play-In slices become BPI1… in order.`;
  const blc = text.match(/^BLC(\d+)$/);
  if (blc)
    return `Last Chance entrant #${blc[1]}. After groups finish, the next-worst slice of the merged remainder becomes BLC1… before two advancers join the eight-team Play-In.`;
...
export function TeamLine({ name, winner, editable, score, onScoreChange, onWin }) {
...
export function MatchCard({ match, editable, scoreDraft, setScoreDraft, clearMatchDraft, submitResult, updateMatch }) {
        ...
        <span className="flex flex-wrap items-center justify-end gap-2">
          ...
          {match.meta?.winToken ? (
            <span
              ...
              title={
                describeBracketToken(match.meta.winToken) ||
